...
def tab_settings():
    ...
    with st.expander("LLM Model Settings", expanded=True):
        st.session_state['llm_temperature'] = st.slider("Temperature", 0.0, 1.0, 0.25, 0.05, help="Set the temperature for LLM responses. Higher values (e.g., 0.8) will make the output more creative, while lower values (e.g., 0.2) will make it more focused and deterministic.")
        st.session_state['llm_top_p'] = st.slider("top-p", 0.0, 1.0, 0.9, 0.05, help="Set the nucleus sampling parameter (top-p) for LLM responses. This controls the diversity of the output by limiting the token selection to a subset of the most probable tokens whose cumulative probability exceeds the specified value.")
