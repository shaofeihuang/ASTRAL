...
def generate_model_aml(arch_narration, threat_model, attack_paths):
    ...
    with st.spinner("Generating AutomationML Model (Step 1) ..."):
        for attempt in range(max_retries):
            try:
                ...
                print ("----------------------------------------------------")
                print (f"[#] Starting AML generation at: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_time))}")
                print (f"[#] Generating AML - Step 1 [Attempt {attempt + 1}]")
                ...
                if attempt == max_retries - 1:
                    st.error(f"Error generating model (Step 1) after {max_retries} attempts: {e}. Stopping execution.")
    ...
    with st.spinner("Generating AutomationML Model (Step 2) ..."):
        for attempt in range(max_retries):
            try:
                ...
                valid_pairs_json = clean_response(valid_pairs_json)  # Clean response to extract JSON content
                end_time = time.time()
                elapsed_secs = end_time - start_time
                ...
                if attempt == max_retries - 1:
                    st.error(f"Error generating model (Step 2) after {max_retries} attempts: {e}. Stopping execution.")
                    st.stop()  # stop execution if final attempt fails
                else:
                    delay = 2 ** attempt + random.uniform(0, 1)
                    st.warning(f"Attempt {attempt + 1} failed, retrying in {delay:.1f} seconds...")
    ...
    with st.spinner("Generating AutomationML Model (Step 3) ..."):
        for attempt in range(max_retries):
            try:
                print (f"[#] Generating AML - Step 3 [Attempt {attempt + 1}]")
                start_time = time.time()
                ...
                if attempt == max_retries - 1:
                    st.error(f"Error generating model (Step 3) after {max_retries} attempts: {e}. Stopping execution.")
                    st.stop()  # stop execution if final attempt fails
                else:
    ...
    with st.spinner("Generating AutomationML Model (Step 4) ..."):
        ...
        for attempt in range(max_retries):
            try:
                print (f"[#] Generating AML - Step 4 (Final) [Attempt {attempt + 1}]")
                start_time = time.time()
                ...
                if attempt == max_retries - 1:
                    st.error(f"Error generating model (Step 4) after {max_retries} attempts: {e}. Stopping execution.")
                    st.stop()  # stop execution if final attempt fails
                else:
                    delay = 2 ** attempt + random.uniform(0, 1)  # exponential backoff with jitter
                    st.warning(f"Attempt {attempt + 1} failed, retrying in {delay:.1f} seconds...")
                    time.sleep(2 * delay) # longer delay before final step retries due to complexity
